...
def submit_limit_order(rit, ticker, order_type, price, quantity):
    """
    Submits a limit order.
    
    :param rit: RIT client object.
...
def calculate_and_submit_orders(rit, ticker):
    ...
    while rit.get_case().status == Case.Status.ACTIVE:
        ...
        if bid_quantity > 0:
            submit_limit_order(rit, ticker, 'bid', bid, bid_quantity)

        if ask_quantity > 0:
            submit_limit_order(rit, ticker, 'ask', ask, ask_quantity)
